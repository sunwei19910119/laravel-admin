...
<!-- Horizontal Form -->
    <div class="box box-info">
        ...
        <!-- form start -->
        <form id="form" class="form-horizontal" action="{{ url('/admin/permission') }}" method="post">
            ...
            <?php echo csrf_field(); ?>
                <button type="button" class="btn btn-cancel pull-left" onclick="location='{{ url("/admin/permission") }}';">返回</button>
                <button type="submit" class="btn btn-success pull-right _submit_" data-form-id="form"
                        data-refresh-url="{{ url("/admin/permission") }}">提交</button>
            </div>
        <!-- /.box-footer -->
        </form>
    </div>
